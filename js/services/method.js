...
async function serverLogin(serverName = "") {
    const session = sdb.getItem(LOGIN_SESSION);
    if (session != null) {
...
async function logintoserver(client = '') {
    ...
    if (response.status == 200) {
        ...
        sdb.setItem(LOGIN_SESSION, JSON.stringify(ss));
        return ss['token'];
    } else {
...
async function requestMovie(token = "", type = "", postId = "") {
    const result = db.getItem(`${postId}`);
    if (result != null) {
        ...
        if (response.status == 200) {
            db.setItem(`${postId}`, JSON.stringify(response.data));
